...
News summarization systems such as Google News and Columbia Newsblaster \cite{McKeown02} follow a pipeline architecture similar to that of WikiTopics. Newsblaster's pipeline comprise components of web crawl, article text extraction, clustering, classification, summarization, and web page generation whereas Google News lacks of summarization system. Both the system are operating real-time processing a constant stream of newswire documents. In contrast, WikiTopics analyzes a static set of articles although the latest revision of an article is extracted as of the day on which the article is selected as though articles are selected every day. Classification and summarization system are next steps for WikiTopics to enhance its clustering and textualization systems. Hierarchical clustering like three-level clustering used in Newsblaster would be adapted to WikiTopics to hierarchical current events. Summarizing multiple sentences that are extracted from the articles in the same cluster would improve coverage of the generate summary about the current event.
...
%% Future work: detecting a newly popular event earlier.
This work has demonstrated that a simple pipeline of article selection, clustering, and sentence selection can be formed to identify and describe significant current events in the manner Wikipedia editors manually summarizes current events with related articles. It is shown that the hand-curated events are not an appropriate objective to predict based on pageviews. Clustering methods based on topic models and inter-article link structure are shown to be useful to group a set of articles that are coherently related to a current event. K-means outperformed LDA with a considerable margin. Clustering based on only link structure achieved comparable performance with clustering based on topic models. In a third of the cases, the sentence that best describe a current event can be extracted from the article text base on temporal expressions in text. Co-reference resolution system assist a generation of text with an improved readability. As future work, ensemble clustering and text classification could be used to improve clustering performance and sentence compression, fusion, and paraphrasing could be applied to selected sentences with various strategies to more succinctly summarize the current events.
