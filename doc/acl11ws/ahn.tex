...
In this paper we analyze a novel dataset: we have collected the hourly page view statistics\footnote{http://dammit.lt/wikistats} for every Wikipedia page in every language for the last three years. We show how these page view statistics along with other features like inter-page hyperlinks can be used to identify and explain popular trends, including popular films and music, sports championships, elections, natural disasters, etc.
...
%% alternatives
There are various viable alternatives to the monthly trend value. As one of them, we did some preliminary experiments with the daily trend value, which is defined by $ d_1^k - d_2^k $, i.e. the difference of the page views between the day and the previous day: we found that articles selected using the daily trend value have little overlap--less than half the articles overlapped. There is much room for future research. Future work will consider the addition of sources other than page views, such as edit histories and Wikipedia category information, along with more intelligent technique to combine these different sources.
...
To see if external source help better clustering, we explored the use of news articles.  We included the news articles that we crawled from various news websites into the same vector space as the Wikipedia articles, and ran K-means clustering with the same settings as the previous ones.  For each day, we parametrized the number of previous days from which the news articles are included.  The results did not show significant improvement over the clustering without news articles.
